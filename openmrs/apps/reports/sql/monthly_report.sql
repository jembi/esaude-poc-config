SELECT * FROM (SELECT
  ...
  JOIN
  (SELECT
          COUNT(CASE WHEN (DATE(lab_orders.consultation_date) BETWEEN '#startDate#' AND '#endDate#' AND TIMESTAMPDIFF(YEAR, tarv_patients.birthdate, CURDATE()) <= 14) THEN 1 ELSE NULL END) AS E11,
          COUNT(CASE WHEN (DATE(lab_orders.consultation_date) BETWEEN '#startDate#' AND '#endDate#' AND TIMESTAMPDIFF(YEAR, tarv_patients.birthdate, CURDATE()) <= 14) THEN 1 ELSE NULL END) AS E12

   FROM (SELECT * FROM (SELECT orders.encounter_id, encounter.patient_id,encounter.encounter_datetime AS consultation_date,concept_name.name AS test_name FROM orders JOIN concept_name ON concept_name.concept_id = orders.concept_id AND concept_name.locale = 'en' AND concept_name.name = 'LO_ViralLoad'
     ...
     (SELECT COUNT(lab_orders.patient_id) FROM (SELECT * FROM (SELECT orders.encounter_id, encounter.patient_id,encounter.encounter_datetime AS consultation_date,concept_name.name AS test_name FROM orders JOIN concept_name ON concept_name.concept_id = orders.concept_id AND concept_name.locale = 'en' AND concept_name.name = 'LO_ViralLoad'
       ...
       JOIN
       (SELECT * FROM person p JOIN (SELECT patient_id,patient_status FROM patient_status_state WHERE id IN (SELECT MAX(id) FROM patient_status_state GROUP BY patient_id) AND patient_status = 'TARV') p_status ON p_status.patient_id= p.person_id) tarv_patients on tarv_patients.person_id = lab_orders.patient_id
       AND DATE(lab_orders.consultation_date) BETWEEN CONCAT((YEAR('#startDate#')-1),'-12-20') AND '#startDate#') = 0) E1
  JOIN
  (SELECT
          COUNT(CASE WHEN TIMESTAMPDIFF(YEAR, tarv_patients.birthdate, CURDATE() <= 14) THEN 1 ELSE NULL END) AS E21,
          COUNT(CASE WHEN TIMESTAMPDIFF(YEAR, tarv_patients.birthdate, CURDATE() >= 15) THEN 1 ELSE NULL END) AS E22,
          COUNT(CASE WHEN TIMESTAMPDIFF(YEAR, tarv_patients.birthdate, CURDATE() <= 14 AND value_numeric < 1000) THEN 1 ELSE NULL END) AS E31,
          COUNT(CASE WHEN TIMESTAMPDIFF(YEAR, tarv_patients.birthdate, CURDATE() >= 15 AND value_numeric < 1000) THEN 1 ELSE NULL END) AS E32
   FROM (SELECT observation.encounter_id, encounter.patient_id,encounter.encounter_datetime as consultation_date, observation.value_numeric, observation.concept_name_type,observation.name FROM (SELECT value_numeric, concept_name_type, name,locale, encounter_id, obs_id,obs.concept_id FROM obs
     JOIN concept_name c ON c.concept_id = obs.concept_id WHERE concept_name_type = 'FULLY_SPECIFIED'
   AND locale = 'en' AND name = 'LO_ViralLoad'  AND obs.value_numeric <> 0 ORDER BY encounter_id DESC) observation
     ...
     AND DATE(vl_results.consultation_date) BETWEEN '#startDate#' AND '#endDate#'
     AND
     ...
     AND locale = 'en' AND name = 'LO_ViralLoad'  AND obs.value_numeric <> 0 ORDER BY encounter_id DESC) observation
       ...
       JOIN
       (SELECT * FROM person p JOIN (SELECT patient_id,patient_status FROM patient_status_state WHERE id IN (SELECT MAX(id) FROM patient_status_state GROUP BY patient_id) AND patient_status = 'TARV') p_status ON p_status.patient_id= p.person_id) tarv_patients on tarv_patients.person_id = vl_results.patient_id
       AND DATE(vl_results.consultation_date) BETWEEN CONCAT((YEAR('#startDate#')-1),'-12-20') AND '#endDate#') = 0) E2
  JOIN
