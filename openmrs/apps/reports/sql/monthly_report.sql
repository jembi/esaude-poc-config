SELECT * FROM (SELECT
  ...
  JOIN
  (select  name AS hf_district from address_hierarchy_entry WHERE address_hierarchy_entry_id = (select parent_id from address_hierarchy_entry join (SELECT REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(property_value,',',1),':',-1),'\"','') as nid FROM global_property WHERE property = 'healthFacility.info' LIMIT 1) NID_table ON address_hierarchy_entry.name Like CONCAT('%',NID_table.nid,'%'))) AS hf_district
  JOIN
  (SELECT address_hierarchy_entry.name AS hf_province FROM address_hierarchy_entry WHERE address_hierarchy_entry_id =(select parent_id from address_hierarchy_entry WHERE address_hierarchy_entry_id = (select parent_id from address_hierarchy_entry join (SELECT REPLACE(SUBSTRING_INDEX(SUBSTRING_INDEX(property_value,',',1),':',-1),'\"','') as nid FROM global_property WHERE property = 'healthFacility.info' LIMIT 1) NID_table ON address_hierarchy_entry.name Like CONCAT('%',NID_table.nid,'%')))) AS hf_province;