...
from
   person p 
	...
	JOIN
      ...
      AND cv.retired = 0 
      and (cv.concept_full_name = 'NEW_PATIENT' OR cv.concept_full_name='TRANSFERRED_PATIENT')
	inner join 
	  concept_view cv2
      on pa.value = cv2.concept_id
      AND cv2.retired = 0
      and pa.voided=0
      and cv2.concept_full_name <> 'Patient_Transferred_Out' AND cv2.concept_full_name<>'Inactive_Death' AND cv2.concept_full_name<>'Patient_Suspension'
	left JOIN 
      person_attribute tdate
      on tdate.person_id=p.person_id
      and tdate.person_attribute_type_id = (
      select person_attribute_type_id from person_attribute_type patype where patype.name='Date_of_transference')
	left JOIN 
	...
	inner join
      ...
      where erp.id = (select max(id) from erpdrug_order 
                      where erpdrug_order.dispensed=1
                            ...
                            and (ddate.value is null or (ddate.value) < DATe(erp.dispensed_date))
                            and (tdate.value is null or (tdate.value) < DATe(erp.dispensed_date))
                            and (sdate.value is null or (sdate.value) < DATe(erp.dispensed_date))
